...
app = FastAPI(title="AgentOS API", version="0.6.1", docs_url="/docs")
...
def vector_literal(vector: list[float]) -> str:
    """Serialize an embedding for PostgreSQL's vector input syntax."""
...
async def health() -> dict[str, Any]:
    return {"status": "ok", "service": "agentos-api", "version": "0.6.1", "database": "connected" if db_available() else "memory", "web_search": bool(os.getenv("TAVILY_API_KEY")), "semantic_retrieval": bool(os.getenv("OPENAI_API_KEY"))}
