...
async def generate_report(req: ResearchRequest, plan: list[str]) -> str:
    ...
    if not api_key or AsyncOpenAI is None:
        return "## Research brief\n\nNo model key is configured. Add `OPENAI_API_KEY` to enable live synthesis.\n\n### Planned reasoning\n" + "\n".join(f"{i+1}. {step}" for i, step in enumerate(plan))
...
async def research(req: ResearchRequest) -> ResearchResponse:
    ...
    for i, step in enumerate(plan):
        runs[run_id]["events"].append({"agent": "orchestrator", "step": i + 1, "event": step, "timestamp": datetime.now(timezone.utc).isoformat()})
    try:
    ...
    return ResearchResponse(id=run_id, status=status, question=req.question, report=report, plan=plan, sources=[], claims=[], metrics={"latency_seconds": round(elapsed, 3), "source_count": 0, "claim_count": 0})
